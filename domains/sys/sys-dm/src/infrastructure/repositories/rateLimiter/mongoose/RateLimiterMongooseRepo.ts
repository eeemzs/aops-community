...
import { DbRateLimiter, IdbRateLimiter } from '../../../db/rateLimiter/mongoose/mongoose.schema.rateLimiter.js';
import { mapperRateLimiterMongoose } from '../../../db/rateLimiter/mongoose/mongoose.mapper.rateLimiter.js';
import {
...
export class RateLimiterMongooeseRepo
  extends MraBase<IbmRateLimiter, IdbRateLimiter>
  implements IRepositoryPortRateLimiter
{
  constructor(deps: { repositoryConfig: RepositoryConfig; logger?: XfLogger }) {
    super(DbRateLimiter, {
      mapper: mapperRateLimiterMongoose,
      logger: deps.logger,
  ...
  newAttempt(key: string, scope: string, rule: RateLimitRule): Effect.Effect<RateLimiterResult, Error> {
    return rateLimiterCommonNewAttempt(
      this.findSingleAdapter,
      this.createAdapter,
      this.updateByIdAdapter,
      this.deleteByIdAdapter,
      key,
      scope,
      rule,
      this.logger
  ...
  private findSingleAdapter: RateLimitFindSingleAdapter<IbmRateLimiter> = async (key: string, scope: string) => {
    const eff = this.findSingle({ key, scope });
  ...
  private deleteManyAdapter: RateLimitDeleteManyAdapter = async (criteria: any) => {
    const eff = criteria.key && criteria.scope
      ? this.deleteMany({ match: { key: criteria.key, scope: criteria.scope } })
      : this.deleteMany({});
    return await Effect.runPromise(
      eff.pipe(
        Effect.match({
  ...
  // Use base cleanupAll (Effect<number, ...>) and let adapter bridge at callsite
}
