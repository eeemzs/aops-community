...
- The verified global core includes a small neutral AOPS router, rich on-demand references and user guides for mounted Community domains (`agentspace`, `chatv3`, `docman`, `projectman`, and `sys`), plus public-safe collaboration and working-discipline references. Setup makes all of these available without silently choosing or activating a working discipline for the user.
- Fileman, Tasker/Runner, and loop guidance must not appear in the default core until those capabilities are actually shipped and mounted. Private development architecture, machine paths, repository caches, and source-workspace recipes must never enter the npm agent-asset payload.
